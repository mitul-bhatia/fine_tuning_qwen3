...
\begin{titlepage}
    ...
    \begin{tikzpicture}[scale=0.75]
        % Top: Base Model
        \node[draw, rounded corners, fill=qwenblue!20, minimum width=4cm, minimum height=1cm] (base) at (0,0) {\textbf{Qwen 2.5 1.5B}};
        
        % Bottom row with proper spacing
        \node[draw, rounded corners, fill=orange!20, minimum width=2.5cm, minimum height=0.9cm] (data) at (-5,-2.5) {\textbf{Dataset}};
        \node[draw, rounded corners, fill=loragreen!20, minimum width=2.5cm, minimum height=0.9cm] (lora) at (0,-2.5) {\textbf{LoRA}};
        \node[draw, rounded corners, fill=purple!20, minimum width=2.5cm, minimum height=0.9cm] (out) at (5,-2.5) {\textbf{Fine-Tuned}};
...
The key insight from the LoRA paper is that weight updates during fine-tuning often have low ``intrinsic rank'':
...
\begin{lstlisting}[language=Python, caption=train.py - Complete Code]
"""
...
{row['output']}<|im_end|>"""
    ...
    return dataset.map(lambda x: format_row(x))
...
\section{Complete Testing Script (test.py)}

\begin{lstlisting}[language=Python, caption=test.py - Complete Code]
"""
...
ft_model = PeftModel.from_pretrained(
    ft_model, ADAPTER_PATH
).to(DEVICE).eval()

def generate(model, question):
    system = "You are a compassionate emotional support assistant."
